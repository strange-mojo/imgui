...
#   endif //FILENAME_MAX!=DIRENT_MAX_PATH
    ...
    void resetVariables() {
        ...
        needCollapseKnownFolders = false;
...
#       endif //IMGUI_USE_MINIZIP
    ...
    inline void calculateBrowsingDataTableSizes(const ImVec2& childWindowSize=ImVec2(-1,-1))    {
        ...
        //without wordwrap on the entries, columns are problematic especially at higher font size
        //investigate proper tile layout with icons?
        int maxNumBrowsingColumns = 1;//(childWindowSize.x>0) ? (childWindowSize.x/100) : 6;
...
ImVec2 Dialog::WindowSize(400,400);
...
#                   endif // IMGUI_USE_MINIZIP
    ...
    if (I.allowKnownDirectoriesSection && pUserKnownDirectories->size()>0)  {
        ...
        if(I.needCollapseKnownFolders) { ImGui::SetNextTreeNodeOpen(false); I.needCollapseKnownFolders = false; }
        if (ImGui::CollapsingHeader("Known Directories##imguifs_UserKnownDirectories"))  {
            ...
            for (int i=0,sz=(int)pUserKnownDirectories->size();i<sz;i++)  {
                ...
                if(wrapMode) {
                    sumX+=ImGui::CalcTextSize(userKnownFolderDisplayName).x;
                    sumX+=2.*ImGui::GetStyle().FramePadding.x;    // needed ?
                    sumX+=2.*ImGui::GetStyle().ItemInnerSpacing.x;
                    if(sumX >= windowWidth) sumX=0;
                    if(i!=0 && sumX>0) ImGui::SameLine();
                }
                else {
                    if (i!=0 && (i>=*pNumberKnownUserDirectoriesExceptDrives || i%7!=6)) ImGui::SameLine();
                }
                if (ImGui::SmallButton(userKnownFolderDisplayName) && strcmp(userKnownFolder,I.currentFolder)!=0) {
                    ...
                    I.forceRescan = true;
                    I.needCollapseKnownFolders = true;
